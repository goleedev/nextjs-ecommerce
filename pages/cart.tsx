          ...
          <div className={styles.body}>
              ...
              {cart.map((item: ProductType, index: number) => (
                <div key={index} className={styles.item}>
                  ...
                  <div className={styles.detail}>
                    <p>{item.product}</p>
                    <p><b>{item.freeShipping ? '무료배송' : '유료배송'}</b> (업제직배송)</p>
                    <p>가격 <b>{item.price.toLocaleString('ko-KR')}</b>원 (할인 50% 적용)</p>
                    <p>총 <b>{item.quantity}</b> 개</p>
                  </div>
            ...
            <div className={styles.total}>
              <div className={styles.totalContent}>
                <p><span>총 상품금액</span> <span>{getTotalPrice().toLocaleString('ko-KR')}원</span></p>
                <p><span>총 배송비</span> <span>0원</span></p>
                <p><span>총 할인금액</span> <span>{getTotalPrice().toLocaleString('ko-KR')}원</span></p>
                <p><span>결제금액</span> <span>{getTotalPrice().toLocaleString('ko-KR')}원</span></p>
              </div>
